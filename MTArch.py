...
class UniTModel(nn.Module):
    def __init__(self, num_gestures, num_persons, d_model=256,layers=[2,2,2,2],block_num=3,enc='xLSTM'):
...
def train_model(layers,block_num,return_para=False,enc='xLSTM'):
    model = UniTModel(act_num, subject_num,layers=layers,block_num=block_num,enc=enc)
    ...
    shared_params = list(model.branch1_before_last.parameters())+ list(model.branch2_before_last.parameters()) +list(model.avgpool.parameters())+list(model.projection.parameters())+list(model.pos_encoder.parameters())+list(model.encoder.parameters())
    ...
    log_name='res/MT'+enc+'_'+str(layers)+'_'+str(block_num)+".csv"
...
for enc in encs:
    for layer in layers:
        for k in block_nums:
            print(layer,k)
            train_model(layer,k,return_para=True,enc=enc)
